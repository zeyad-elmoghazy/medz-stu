...
const LABEL: React.CSSProperties = { fontSize: 11, fontWeight: 600, color: '#8B98A6', marginBottom: 8 };
...
export function AdminUploadWizard({ onContentChanged }: Props) {
  ...
  if (destLoading) {
    return <div style={{ ...CARD, textAlign: 'center', color: '#8B98A6' }}>Loading modules…</div>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              >
                <div
                  style={{
                    ...
                    fontSize: 12,
                    fontWeight: 800,
                    background: active || done ? 'linear-gradient(135deg,#00A6A6,#33BFBF)' : 'rgba(255,255,255,0.04)',
                    color: active || done ? '#F7F9FA' : '#8B98A6',
                ...
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: active || done ? '#F7F9FA' : '#8B98A6' }}>
                    {st.label}
                  </div>
                  <div style={{ fontSize: 9.5, color: '#8B98A6' }}>{st.hint}</div>
                </div>
              </button>
              {idx < stepDefs.length - 1 && (
                <div style={{ flex: 1, height: 2, margin: '0 16px', background: done ? '#00A6A6' : 'rgba(255,255,255,0.1)' }} />
              )}
            </div>
          );
        })}
      ...
      {step === 1 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, alignItems: 'start' }}>
              ...
              {modules.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!m.is_active && <span style={{ fontSize: 9, color: '#8B98A6' }}>inactive</span>}
                  </button>
                );
              })}
            </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ fontSize: 11, color: '#8B98A6', marginTop: 4, marginBottom: 18 }}>
              In <b style={{ color: '#33BFBF' }}>{currentModule?.code ?? '—'}</b>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 340, overflow: 'auto' }}>
              {subjects.length === 0 && (
                <div style={{ fontSize: 12, color: '#8B98A6' }}>No subjects assigned to this module.</div>
              )}
              {subjects.map((s) => {
          ...
          <div style={CARD}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: '#8B98A6', marginTop: 4 }}>
                  In <b style={{ color: '#33BFBF' }}>{currentSubject?.name ?? '—'}</b>
                </div>
              </div>
            ...
            {addingChapter && (
              <div style={{ marginTop: 16, padding: 16, background: '#132B45', border: '1px dashed rgba(0,166,166,0.4)', borderRadius: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#33BFBF', marginBottom: 9 }}>New chapter name</div>
                <div style={{ display: 'flex', gap: 9 }}>
                  <input
                    value={newChapterName}
                    onChange={(e) => setNewChapterName(e.target.value)}
                    placeholder="e.g. Motor Nervous System"
                    style={{ flex: 1, background: '#132B45', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 9, padding: '11px 13px', color: '#F7F9FA', fontSize: 13, fontFamily: 'inherit' }}
                  />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 16, maxHeight: 260, overflow: 'auto' }}>
              {chapters.length === 0 && !addingChapter && (
                <div style={{ fontSize: 12, color: '#8B98A6' }}>No chapters yet for this subject.</div>
              )}
              {chapters.map((c) => {
                ...
                return (
                  <button key={c.id} type="button" onClick={() => setChapterId(c.id)} style={optionBtn(selected)}>
                    ...
                    <span style={{ fontSize: 10, color: '#8B98A6' }}>{c.published_count}/{c.question_count} pub</span>
                    {selected && <span style={{ fontSize: 13, color: '#00A6A6' }}>✓</span>}
                  </button>
                );
              })}
            </div>
      ...
      {step === 2 && (
        <div>
          <div style={{ fontSize: 13, color: '#8B98A6', marginBottom: 16 }}>
            Publishing to <b style={{ color: '#F7F9FA' }}>{moduleCode} · {currentSubject?.name} · {currentChapter?.name}</b> — how would you like to add questions?
          </div>
      ...
      {step === 3 && mode === 'manual' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 22, alignItems: 'start' }}>
          <div style={CARD}>
            ...
            <div style={{ fontSize: 11, color: '#8B98A6', marginBottom: 20 }}>{moduleCode} · {currentChapter?.name ?? '—'}</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '20px 0 10px' }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#8B98A6' }}>Answer options</div>
              <button type="button" onClick={() => mqOptions.length < 5 && setMqOptions([...mqOptions, ''])} disabled={mqOptions.length >= 5} style={{ fontSize: 11, fontWeight: 700, color: mqOptions.length >= 5 ? '#8B98A6' : '#00A6A6', cursor: mqOptions.length >= 5 ? 'default' : 'pointer', background: 'transparent', border: 'none', fontFamily: 'inherit' }}>
                + Add option
              </button>
            </div>
              ...
              {mqOptions.map((val, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                    ...
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, fontWeight: 700, color: '#8B98A6', width: 14, flex: 'none' }}>{OPTION_LETTERS[i]}</span>
                    <input
                      value={val}
                    ...
                    />
                    {mqOptions.length > 2 && (
                      <button type="button" onClick={() => { const next = mqOptions.filter((_, j) => j !== i); let c = mqCorrect; if (c >= next.length) c = next.length - 1; setMqOptions(next); setMqCorrect(c); }} style={{ width: 30, height: 30, flex: 'none', borderRadius: 8, display: 'grid', placeItems: 'center', fontSize: 15, color: '#8B98A6', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', background: 'transparent', fontFamily: 'inherit' }}>×</button>
                    )}
                  </div>
                );
              })}
            ...
            <div style={{ fontSize: 10, color: '#8B98A6', marginTop: 6 }}>
            ...
            <div style={{ marginTop: 20, display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {(['easy', 'medium', 'hard'] as const).map((d) => (
                  <button key={d} type="button" onClick={() => setMqDifficulty(d)} style={{ fontSize: 11, fontWeight: 700, textTransform: 'capitalize', padding: '6px 12px', borderRadius: 8, cursor: 'pointer', color: mqDifficulty === d ? '#F7F9FA' : '#8B98A6', background: mqDifficulty === d ? 'rgba(0,166,166,0.16)' : 'transparent', border: `1px solid ${mqDifficulty === d ? 'rgba(0,166,166,0.4)' : 'rgba(255,255,255,0.08)'}`, fontFamily: 'inherit' }}>{d}</button>
                ))}
              </div>
            </div>
            ...
            <div style={reviewGateBox}>
              ...
              <span>This question will be saved as <b style={{ color: '#F7F9FA' }}>under review</b>. Approve it from the Review Queue to publish.</span>
            </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ fontSize: 32, fontWeight: 800, color: '#00A6A6', marginBottom: 4 }}>{sessionDrafts.length}</div>
            <div style={{ fontSize: 10, color: '#8B98A6', marginBottom: 18 }}>{sessionDrafts.length === 1 ? 'question saved' : 'questions saved'}</div>
            {sessionDrafts.slice(0, 5).map((d) => (
              <div key={d.id} style={{ fontSize: 11.5, color: '#8B98A6', padding: '10px 12px', background: '#132B45', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 9, marginBottom: 8 }}>
      ...
      {step === 3 && mode === 'pdf' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 22 }}>
          <div style={CARD}>
              ...
              <>
                ...
                {pdfPhase === 'failed' && pdfError && (
                  <div role="alert" style={{ ...errorBox, marginTop: 14 }}>
                    {pdfError}
                    {pdfJobId && (
                      <div style={{ marginTop: 6, fontSize: 10, color: '#8B98A6' }}>
            ...
            {pdfPhase === 'processing' && (
              <div style={{ fontSize: 13, color: '#8B98A6' }}>Extracting on the server…</div>
            )}

            {pdfPhase === 'done' && (
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#10B981', marginBottom: 8 }}>✓ Extraction complete</div>
                <div style={{ fontSize: 12, color: '#8B98A6', marginBottom: 20 }}>
                  <b style={{ color: '#F7F9FA' }}>{pdfExtracted}</b> question{pdfExtracted === 1 ? '' : 's'} extracted, queued as <b style={{ color: '#33BFBF' }}>under review</b>.
                </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ fontSize: 11, color: '#8B98A6', marginBottom: 16, lineHeight: 1.5 }}>
      ...
      {step === 3 && mode === 'json' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 22 }}>
          <div style={CARD}>
            ...
            {jsonPhase === 'processing' && <div style={{ fontSize: 13, color: '#8B98A6' }}>Importing…</div>}

            {jsonPhase === 'done' && (
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#10B981', marginBottom: 8 }}>✓ Import complete</div>
                <div style={{ fontSize: 12, color: '#8B98A6', marginBottom: 20 }}>
                  <b style={{ color: '#F7F9FA' }}>{jsonImported}</b> question{jsonImported === 1 ? '' : 's'} created or updated, queued as <b style={{ color: '#33BFBF' }}>under review</b>.
                </div>
  ...
  return (
    <button
      ...
      style={{
        position: 'relative',
        overflow: 'hidden',
        background: highlight ? 'linear-gradient(160deg,#132B45,#0B1F33)' : '#132B45',
        border: `1px solid ${highlight ? 'rgba(0,166,166,0.5)' : 'rgba(255,255,255,0.1)'}`,
        borderRadius: 18,
        padding: 26,
        cursor: 'pointer',
        textAlign: 'left',
        fontFamily: 'inherit',
        color: 'inherit',
        boxShadow: highlight ? '0 0 0 1px rgba(0,166,166,0.2), 0 0 30px rgba(0,166,166,0.12)' : 'none',
      }}
    >
      <div style={{ width: 48, height: 48, borderRadius: 14, background: highlight ? 'rgba(0,166,166,0.18)' : 'rgba(255,255,255,0.05)', border: `1px solid ${highlight ? 'rgba(0,166,166,0.4)' : 'rgba(255,255,255,0.1)'}`, display: 'grid', placeItems: 'center', fontSize: 22, marginBottom: 16 }}>
        {emoji}
      </div>
      <div style={{ fontSize: 16, fontWeight: 800, letterSpacing: '-0.01em' }}>{title}</div>
      <p style={{ fontSize: 12, color: '#8B98A6', margin: '9px 0 16px', lineHeight: 1.5 }}>{desc}</p>
      <div style={{ textAlign: 'center', fontSize: 12.5, fontWeight: 700, color: highlight ? '#F7F9FA' : '#F7F9FA', background: highlight ? 'linear-gradient(135deg,#00A6A6,#33BFBF)' : 'transparent', border: highlight ? 'none' : '1px solid rgba(255,255,255,0.14)', padding: 11, borderRadius: 11, boxShadow: highlight ? '0 0 20px rgba(0,166,166,0.4)' : 'none' }}>
        {cta}
      </div>
    </button>
  );
...
function FileDropzone({
  ...
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 8, cursor: 'pointer', padding: '18px 16px', border: `1.5px dashed ${file ? 'rgba(16,185,129,0.5)' : 'rgba(0,166,166,0.4)'}`, borderRadius: 12, background: file ? 'rgba(16,185,129,0.06)' : 'rgba(0,166,166,0.06)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 12, fontWeight: 700 }}>{label}</div>
        <span style={{ fontSize: 9, color: required ? '#EF4444' : '#8B98A6', fontWeight: 700 }}>{required ? 'REQUIRED' : 'OPTIONAL'}</span>
      </div>
      <div style={{ fontSize: 11, color: '#8B98A6' }}>{hint}</div>
      {file ? (
        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: '#10B981' }}>
          <span>📄 {file.name}</span>
          <button type="button" onClick={(e) => { e.preventDefault(); onFile(null); }} style={{ fontSize: 10, color: '#8B98A6', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>× remove</button>
        </div>
      ) : (
        <div style={{ fontSize: 11, color: '#8B98A6', marginTop: 6 }}>Click to choose a file (up to 10 MB)</div>
      )}
      <input
        type="file"
        accept={accept}
...
function optionBtn(selected: boolean): React.CSSProperties {
  return {
    ...
    border: `1px solid ${selected ? 'rgba(0,166,166,0.5)' : 'rgba(255,255,255,0.06)'}`,
    textAlign: 'left',
    fontFamily: 'inherit',
    color: 'inherit',
    width: '100%',
...
const primaryBtn: React.CSSProperties = {
  ...
  padding: '13px 26px',
  borderRadius: 12,
  boxShadow: '0 0 22px rgba(0,166,166,0.4)',
  border: 'none',
  fontFamily: 'inherit',
};
...
const bigPrimaryBtn: React.CSSProperties = {
  ...
  color: '#F7F9FA',
  background: 'linear-gradient(135deg,#00A6A6,#33BFBF)',
  boxShadow: '0 0 20px rgba(0,166,166,0.35)',
  border: 'none',
  width: '100%',
  fontFamily: 'inherit',
};
